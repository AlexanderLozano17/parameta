...
public class RoleController {
	...
	@GetMapping("/{id}")
	public ResponseEntity<ResponseApi<RoleDTO>> findRoleById(@PathVariable Long id) {
		 logger.info(LogHelper.start(getClass(), "findRoleById"));
		 
		try {
			Optional<RoleDTO> rol = rolService.findRoleById(id);
			if (rol.isPresent()) {
				logger.info(LogHelper.success(getClass(), "findRoleById", String.format(LogMessages.ENTITY_FOUND, id)));
				logger.info(LogHelper.end(getClass(), "findRoleById"));
				return ResponseEntity.ok(new ResponseApi(ApiMessages.SUCCESS, ApiMessages.RECORD_FOUND, rol.get()));
			} 
			...
			logger.error(LogHelper.error(getClass(), "findRoleById", e.getMessage()), e);
			logger.info(LogHelper.end(getClass(), "findRoleById"));
